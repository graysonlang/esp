...
1. **esbuild writes `sources` relative to the output directory.** With `outdir: www` sitting beside the `src/` and `app/` roots, every entry comes out as `../src/...` / `../app/...`. This is pure relative-path arithmetic and is spec-correct: per the Source Map v3 spec, `sources` resolve relative to the `.map` file's own location, and `www/main.js.map` genuinely reaches `src/` by going up one level.

   ```
   outdir: "www"  ->  sources: ["../src/util.ts", "../app/main.ts"]
...
3. **The dev server serves `www/` as the web root, and vscode-js-debug resolves a source by joining it onto `webRoot`, keeping the `..`.** With `webRoot: "${workspaceFolder}"`, `../src/util.ts` resolves to `<workspaceFolder>/../src/util.ts` — one level *above* the repo — so the file isn't found and the breakpoint can't bind. `webRoot: "${workspaceFolder}/src"` only "works" because `…/src/../src/util.ts` collapses back to `…/src/util.ts` (and `…/src/../app/main.ts` to `…/app/main.ts`); it cancels the `../` and nothing more.

By contrast, Vite binds with `webRoot: "${workspaceFolder}"` because its dev server serves the source files themselves at `/src/...` URLs — the served tree mirrors disk from the project root. esp deliberately serves only `www/` (a clean artifact boundary that mirrors production), which is what exposes the offset.
...
The offset between "`www/` is the web root" and "the sources live above `www/`" has to be absorbed somewhere. There is no free lunch — only a choice of which layer owns it:
...
| **Serve shape** (esp) | Serve the project root and load `/www/main.js`, so the served tree mirrors disk and `../src` resolves correctly. | Fully spec-correct, no plugin. But the dev server then exposes the whole tree (`node_modules`, `.git`, raw sources), defeating the clean output-only boundary. |
...
```jsonc
{
  ...
  // esbuild emits sources as `../src/...` (relative to outdir `www/`). Strip the
